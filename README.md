...
本项目为 OpenHarmony 平台编译了 autoconf，并发布预构建包。
...
```sh
cd /opt
curl -fLO https://github.com/Harmonybrew/ohos-autoconf/releases/download/2.72/autoconf-2.72-ohos-arm64.tar.gz
curl -fLO https://github.com/Harmonybrew/ohos-perl/releases/download/5.42.0/perl-5.42.0-ohos-arm64.tar.gz
curl -fLO https://github.com/Harmonybrew/ohos-m4/releases/download/1.4.20/m4-1.4.20-ohos-arm64.tar.gz
tar -zxf autoconf-2.72-ohos-arm64.tar.gz
...
这个项目使用本地编译（native compilation，也可以叫本机编译或原生编译）的做法来编译鸿蒙版 autoconf，而不是交叉编译。

需要在 [鸿蒙容器](https://github.com/hqzing/dockerharmony) 中运行项目里的 build.sh，以实现 autoconf 的本地编译。
